...
const createToastId = () => globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`;
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm leading-6 font-bold">{toast.title}</p>
          {toast.description && (
        ...
        <button
          ...
          className="grid size-8 shrink-0 place-items-center rounded-lg text-lg opacity-65 transition outline-none hover:bg-black/5 hover:opacity-100 focus-visible:ring-2 focus-visible:ring-black/20"
...
export const ToastProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    if (toasts.length > 0 && !el.open) {
      el.showModal();
    } else if (toasts.length === 0 && el.open) {
      el.close();
    }
  }, [toasts.length]);
  ...
  return (
    <ToastContext.Provider value={value}>
      ...
      <dialog
        ref={dialogRef}
        className="pointer-events-none m-0 max-h-none max-w-none border-0 bg-transparent p-0 backdrop:bg-transparent"
      >
        <div
          dir="rtl"
          className="pointer-events-none fixed inset-x-3 top-3 flex flex-col gap-3 sm:inset-x-auto sm:start-4 sm:w-[min(24rem,calc(100vw-2rem))]"
          aria-live="polite"
          aria-atomic="true"
        >
